...
module axi_dmac_burst_memory #(
  ...
  output reg [BYTES_PER_BURST_WIDTH-1:0] dest_data_burst_length = 'h00,
  output reg dest_data_burst_partial = 'b0,
  output reg [ID_WIDTH-1:0] dest_data_id = 'h00,
...
always @(posedge dest_clk) begin
  if (dest_last_beat == 1'b1) begin
    dest_data_burst_length <= dest_burst_len_data[BYTES_PER_BURST_WIDTH-1:0];
    dest_data_burst_partial <= dest_burst_len_data[BYTES_PER_BURST_WIDTH];
    dest_data_id <= dest_id;
  end
end
...
generate if (DATA_WIDTH_SRC < DATA_WIDTH_DEST) begin
  ...
  // Keep counting the number of source beats in a packed destination beat
  always @(posedge src_clk) begin
    if (src_mem_data_last == 1'b1) begin
  ...
  assign src_burst_len_data = {src_data_partial_burst_d,src_beat_counter,src_num_beats,src_data_valid_bytes_d};

end else begin

  assign src_burst_len_data = {src_data_partial_burst,src_beat_counter,src_data_valid_bytes};
